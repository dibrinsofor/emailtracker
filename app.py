    ...
    for company in companies:
        ...
        while len(years) > count:
            if type(years[count]) == str:
                company_df = csv[
                    (csv["Tracking Service"] == company) & (csv["Date"] == years[count])
                ]
            elif type(years[count]) == int:
                company_df = csv[
                    (csv["Tracking Service"] == company) & (csv["Year"] == years[count])
                ]
...
def unsecure_plot(email_md5):
    ...
    top_5 = (
        df.drop(["Years"], axis=1).sum(axis=0, skipna=True).nlargest(5).index.to_list()
    )
...
def total_plot(email_md5):
    ...
    print(f"secure rows => {secure_rows}\n")
    print(f"unsecure rows => {unsecure_rows}\n")
...
def additional_data(email_md5):
    ...
    df = pandas.DataFrame.from_dict(csv_results)
    top_1_company = (
        df.drop(["Years"], axis=1).sum(axis=0, skipna=True).nlargest(1).index.to_list()
    )
...
def generate_table(email_md5):
    ...
    for company in companies:
        ...
        tracking_services = csv.query(f'Domain == "{company}"')[
            "Tracking Service"
        ].unique()

        results["Company"].extend([company])
        results["Emails Sent"].extend([freq])
        results["Most Common Ad Tracking Services Found"].extend(
            [list(tracking_services)]
        )
            ...
            go.Table(
                ...
                cells=dict(
                    values=[
                        ...
                        df.Company,
                        df["Emails Sent"],
                        df["Most Common Ad Tracking Services Found"],
                    ],
                    align="left",
                    fill_color="white",
    ...
    fig.update_layout(
        width=900, height=410, paper_bgcolor="white", plot_bgcolor="black"
    )
